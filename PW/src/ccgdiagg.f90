...
!
#ifdef USE_CUDA
  real(8) function cgddot( n, x, incx, y, incy )
    !
    USE kinds, ONLY : DP
    !
    IMPLICIT NONE
...
!----------------------------------------------------------------------------
  ...
  DO m = 1, nbnd
     ...
     !
     e(m) = cgddot( kdim2, psi(1,m), 1, hpsi, 1 )
     !
     ...
     iterate: DO iter = 1, maxter
        ...
        !
        es(1) = cgddot( kdim2, spsi(1), 1, g(1), 1 )
        es(2) = cgddot( kdim2, spsi(1), 1, ppsi(1), 1 )
        !
        ...
        !
        cg0 = cgddot( kdim2, cg(1), 1, scg(1), 1 )
        !
        ...
        ! ...                           <y(t)|P^2S|y(t)> = 1
        !
        a0 = 2.D0 * cgddot( kdim2, psi(1,m), 1, ppsi(1), 1 ) / cg0
        !
        CALL mp_sum(  a0 , intra_bgrp_comm )
        !
        b0 = cgddot( kdim2, cg(1), 1, ppsi(1), 1 ) / cg0**2
        !
